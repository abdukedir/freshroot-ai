...
export default function Coach() {
  ...
  const wellnessScore = unifiedWellnessScore({ moodEntries: moodCheckins, pregnancyProfile, breathingSessions: mindfulnessSessions, nutritionScore: 84, marketplaceFreshness: 88 });
  const wellnessRecommendations = generateWellnessRecommendations({ moodEntries: moodCheckins, pregnancyProfile, breathingSessions: mindfulnessSessions, products }).slice(0, 2);
    ...
    <div className="mx-auto grid max-w-6xl gap-6 px-4 py-8 lg:grid-cols-[330px_1fr]">
      ...
      <aside className="panel h-fit p-5">
        ...
        <div className="mt-5 rounded-lg bg-green-50 p-4">
          <p className="text-sm font-black text-primary">Unified wellness score: {wellnessScore}/100</p>
          <div className="mt-2 space-y-2">
            {wellnessRecommendations.map((item) => <p key={item} className="text-xs leading-5 text-slate-700">{item}</p>)}
          </div>
          <Link to="/wellness" className="mt-3 inline-flex text-sm font-black text-primary">Open wellness hub</Link>
        </div>
