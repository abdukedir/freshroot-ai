...
export default function Cart() {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8">
      <h1 className="text-3xl font-black text-dark">{t('cart')}</h1>
      {cart.length === 0 ? (
        <div className="panel mt-6 p-8 text-center"><ShoppingCart className="mx-auto text-primary" size={44} /><p className="mt-4 font-bold text-slate-600">{t('cartEmpty')}</p><Link className="btn-primary mt-5" to="/marketplace">{t('browseMarketplace')}</Link></div>
      ) : (
        <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_320px]">
          ...
          <aside className="panel h-fit space-y-4 p-5"><h2 className="text-xl font-black">{t('orderSummary')}</h2><div className="flex justify-between text-slate-600"><span>{t('subtotal')}</span><strong>{total} ETB</strong></div><div className="flex justify-between text-slate-600"><span>{t('delivery')}</span><strong>45 ETB</strong></div><div className="flex justify-between border-t pt-4 text-lg font-black"><span>{t('total')}</span><span>{total + 45} ETB</span></div><Link to="/checkout" className="btn-primary w-full">{t('checkout')}</Link></aside>
        </div>
      )}
    </div>
  );
