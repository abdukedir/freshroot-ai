  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8">
      <div className="panel flex flex-col gap-4 p-6 md:flex-row md:items-center">
        ...
        <Link to="/wellness" className="btn-secondary"><Brain size={18} />{t('wellnessScore', { score: wellnessScore })}</Link>
        <Link to="/marketplace" className="btn-primary">{t('shopAgain')}</Link>
      </div>
      <section className="panel mt-6 p-5">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="font-bold text-accent">{t('unifiedWellnessProfile')}</p>
            <h2 className="text-2xl font-black text-dark">{t('profileWellnessHeadline')}</h2>
            <p className="mt-2 text-sm leading-6 text-slate-600">{topRecommendation}</p>
          </div>
          <Link to="/wellness" className="btn-secondary shrink-0">{t('viewDashboard')}</Link>
        </div>
      </section>
      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        <section className="panel overflow-hidden">
          <div className="flex items-center gap-2 border-b p-4"><PackageCheck className="text-primary" /><h2 className="font-black">{t('orders')}</h2></div>
        ...
        <section className="panel overflow-hidden">
          <div className="flex items-center gap-2 border-b p-4"><Heart className="text-accent" /><h2 className="font-black">{t('savedProducts')}</h2></div>
          ...
          {!savedProducts.length && <p className="p-4 text-slate-600">{t('noSavedProducts')}</p>}
        </section>
      </div>
    </div>
  );
